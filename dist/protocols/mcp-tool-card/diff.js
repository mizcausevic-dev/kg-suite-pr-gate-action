...
const PII_RANK = { none: 0, low: 1, medium: 2, high: 3 };
const SECRET_RANK = {
    none: 0,
...
export function diffToolCards(previous, next, _opts = {}) {
    ...
    if (sed > 0)
        push("side-effect-class-escalated", `${previous.safety.side_effect_class} → ${next.safety.side_effect_class}`);
    else if (sed < 0)
        push("side-effect-class-relaxed", `${previous.safety.side_effect_class} → ${next.safety.side_effect_class}`);
    if (previous.safety.reversible && !next.safety.reversible)
        push("reversible-flipped-false");
    if (!previous.safety.reversible && next.safety.reversible)
        push("reversible-flipped-true");
    if (previous.safety.rate_limited && !next.safety.rate_limited)
        push("rate-limit-removed");
    if (!previous.safety.rate_limited && next.safety.rate_limited)
        push("rate-limit-added");
    const piiD = PII_RANK[next.safety.pii_exposure] - PII_RANK[previous.safety.pii_exposure];
    if (piiD > 0)
        push("pii-exposure-escalated", `${previous.safety.pii_exposure} → ${next.safety.pii_exposure}`);
    else if (piiD < 0)
        push("pii-exposure-relaxed", `${previous.safety.pii_exposure} → ${next.safety.pii_exposure}`);
    const secD = SECRET_RANK[next.safety.secrets_exposure] - SECRET_RANK[previous.safety.secrets_exposure];
    if (secD > 0)
        push("secrets-exposure-escalated", `${previous.safety.secrets_exposure} → ${next.safety.secrets_exposure}`);
    else if (secD < 0)
        push("secrets-exposure-relaxed", `${previous.safety.secrets_exposure} → ${next.safety.secrets_exposure}`);
    if (previous.safety.human_approval_required && !next.safety.human_approval_required)
        push("human-approval-removed");
    if (!previous.safety.human_approval_required && next.safety.human_approval_required)
        push("human-approval-added");
    // ─── external systems / refusal modes ──────────────────────────────────
    const xs = diffSet(previous.safety.external_systems, next.safety.external_systems);
    for (const s of xs.added)
        push("external-system-added", s);
    for (const s of xs.removed)
        push("external-system-removed", s);
    const rm = diffSet(previous.safety.refusal_modes, next.safety.refusal_modes);
    for (const s of rm.added)
        push("refusal-mode-added", s);
    for (const s of rm.removed)
        push("refusal-mode-removed", s);
    // ─── schema ────────────────────────────────────────────────────────────
    ...
    for (const k of provDiff.removed)
        push("tested-with-provider-removed", k);
    ...
    for (const s of auditLogs.removed)
        push("audit-log-location-removed", s);
    if (previous.audit?.retention_days !== undefined &&
...
function assertCard(card, side) {
    if (!card || typeof card !== "object")
        throw new Error(`${side} must be a ToolCard object`);
    if (!card.tool)
        throw new Error(`${side}.tool is required`);
    if (!card.safety)
        throw new Error(`${side}.safety is required`);
    if (!card.schema)
        throw new Error(`${side}.schema is required`);
}
