import type { BundleDiff, ChangeReason } from "./types.js";
...
export function toMarkdown(diff: BundleDiff): string {
  if (diff.changes.length === 0) return `**No changes.** Manifests are equivalent.`;
  const lines: string[] = [];
...
export function toSummary(diff: BundleDiff): string {
